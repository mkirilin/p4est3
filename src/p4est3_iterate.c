...
typedef struct p4est3_search_area
{
  ...
  p4est3_gloidx       remote_first;     /* Id of the first remote proc that
                                           ...
                                           quadrants */
  p4est3_gloidx       remote_last;      /* Id of the last remote proc that
                                           potentially contains the neighbor
                                           quadrants */
                                ...
                                int *is_lower)
{
  ...
  for (side = 0; side < search_area->nsides; ++side) {
    Level_face[side] = 0;
    is_refine[side] = 1;
    search_area->child_id_face[side] = 0;
    SC3E (sc3_array_resize (idx_f_stack[side], 1));
    SC3E (sc3_array_index (idx_f_stack[side], 0, &arr));
    SC3E (sc3_array_index (*(sc3_array_t **) arr, 0, &begin_face));
    SC3E (sc3_array_index (*(sc3_array_t **) arr, 1, &end_face));
    *(begin_face) =
      SC3_MAX (p3->gtroffset[tree_ids[side]], p3->goffset[p3->mpirank])
              - p3->goffset[p3->mpirank];
    *(end_face) =
      SC3_MIN (p3->gtroffset[tree_ids[side] + 1], p3->goffset[p3->mpirank + 1])
              - p3->goffset[p3->mpirank];
  }
  *is_lower = 0;
  ...
  *end = SC3_MIN (p3->gtroffset[tree + 1], p3->goffset[p3->mpirank + 1])
  ...
  SC3E (p4est3_find_partition
        (p3->alloc, p3->mpisize, p3->goffset, p3->gtroffset[tree],
         p3->gtroffset[tree + 1] - 1, &sa->remote_first, &sa->remote_last));
  if (p3->goffset[sa->remote_first] > p3->gtroffset[tree]) {
    sa->remote_first--;
  }
  if (p3->goffset[sa->remote_last] > p3->gtroffset[tree + 1] - 1) {
    sa->remote_last--;
  }
  /* Make necessary allocs for remote indices stacks when necessary */
